...
class Model():
    def __init__(self, args, infer=False): # infer is set to true during sampling.
        ...
        # TAC https://stackoverflow.com/questions/36078411/tensorflow-are-my-logits-in-the-right-format-for-cross-entropy-function
        # TAC https://stackoverflow.com/questions/34240703/what-is-logits-softmax-and-softmax-cross-entropy-with-logits
        ...
        if infer:
            ...
            # implicitly represented by the target characters against the probability distributions in logits.
            ...
            if self.args.training_loss == 'original':
                eprint('original loss')
            ...
            elif self.args.training_loss == 'hinge':
                eprint('hinge_loss')
                labels = tf.reshape(self.targets, [-1])
                logits = tf.reduce_sum(self.logits, axis=1)
                loss =   tf.reduce_mean(tf.losses.hinge_loss(labels=labels, logits=logits))

            # https://www.tensorflow.org/api_docs/python/tf/losses/softmax_cross_entropy
            elif self.args.training_loss == 'softx':
                eprint('softmax_cross_entropy')
                labels = tf.reshape(self.targets, [-1])
                logits = tf.reduce_sum(self.logits, axis=1)
                loss = tf.losses.softmax_cross_entropy(labels,logits)

            # https://www.tensorflow.org/api_docs/python/tf/losses/sigmoid_cross_entropy
            elif self.args.training_loss == 'sigmx':
                eprint('softmax_cross_entropy')
                labels = tf.reshape(self.targets, [-1])
                logits = tf.reduce_sum(self.logits, axis=1)
            ...
            elif self.args.training_loss == 'softxl':
                eprint('sigmoid_cross_entropy_with_logits')
                labels = tf.reshape(self.targets, [-1])
            ...
            elif self.args.training_loss == 'sigmxl':
                eprint('sigmoid_cross_entropy_with_logits')
                labels = tf.reshape(self.targets, [-1])
                labels = tf.cast(labels,tf.float32)
                logits = tf.reduce_sum(self.logits, axis=1)
            ...
            elif self.args.training_loss == 'wxe':
                eprint('weighted_cross_entropy_with_logits')
                labels = tf.reshape(self.targets, [-1])
                labels = tf.cast(labels,tf.float32)
                logits = tf.reduce_sum(self.logits, axis=1)
                # what to use for pos_weight???
                loss = tf.nn.weighted_cross_entropy_with_logits(labels,logits,pos_weight=.5)
#     targets,
#     logits,
#     pos_weight,
#     name=None
# )
            ...
            else : assert False, 'need to specify training loss'

            # self.targets = tf.one_hot(self.targets, 1)
            # loss = nn_ops.softmax_cross_entropy_with_logits(
            #     labels=tf.reshape(self.targets, [-1]), logits=self.logits)

            # print (np.shape(self.logits))
            # self.probs = tf.nn.softmax(self.logits)
            # print (np.shape(np.mean(self.probs))
            # exit()
            # loss = tf.contrib.seq2seq.sequence_loss(
            #     self.logits, self.targets, output)
