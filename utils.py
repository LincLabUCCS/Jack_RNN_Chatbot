...
def get_args():
    ...
    parser.add_argument('--relevance', type=float, default=.5,
                       help='amount of "relevance masking set to <0 to disable relevance masking')
    
    parser.add_argument('--loss_model', type=str,
                        help='loss model',
                        choices=['MMI','NET','ENT','NORM'],
                        default='MMI'
                        )

    parser.add_argument('--test_mode', type=str,
                        help='loss model',
                        choices=['QA','1CHAT','2CHAT'],
                        default='QA'
                        )

    parser.add_argument('--initial_text', type=str,
                        help='initial text for dialog starter',
                        default='hello world what\'s your name?'
                        )
    ...
    parser.add_argument('--count', type=int, default=50,
                        help='Count for number of tests.')


    args = parser.parse_args()

    # reality check
    if args.loss_model in ['ENT','NET']:
        eprint('Setting relevance to negative value')
        args.relevance = -1

    args.max_length = args.n 

    config_path, freqs_path = get_paths(args.save_dir)

    checkpoint = tf.train.get_checkpoint_state(args.save_dir)
    args.model_path = checkpoint.model_checkpoint_path
    ...
    with open(config_path, 'rb') as f:
        args.saved_args = pickle.load(f)

    # load character frequency file from the model path.
    with open(freqs_path, 'rb') as f:
        args.freqs = pickle.load(f)

    args.mask_reset_token = ord('\n')
    args.forbidden_token  = ord('>')
    return args   

def get_paths(input_path):
    config_path = os.path.join(input_path, 'config.pkl')
    frequencies_path = os.path.join(input_path, 'frequencies.pkl')
    return (config_path, frequencies_path)
    ...
    # return model_path, os.path.join(save_dir, 'config.pkl'), os.path.join(model_path, 'frequencies.pkl')

class Samples():
    def __init__(self,count=100,filename='sample-lines.txt'):
        f = open(filename,'r')
        content = f.readlines()
        self.content = np.random.choice(content,count)

    def get(self):
        while True:
            ret = np.random.choice(self.content)
            if ret.startswith('> '):
                ret = ret[2:]
            tag = '\n'
            if ret.endswith(tag):
...
class TextLoader():
    ...
    def _load_preprocessed(self, tensor_index):
        ...
        if tensor_index == self.tensor_index:
            return
        eprint("loading tensor data file {}".format(tensor_index))
